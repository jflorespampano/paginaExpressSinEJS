const url = 'tipoPartes';
    ...
    /**
     * busca un registro y lo pone en #datosTabla
     * @param {*} dab id del dato a buscar
     */
    function buscaDato(dab) {
        // let parametros = `?id=${dab}`;
        // const localUrl=url+'/one'+parametros
        const localUrl=`${url}/one?id=${dab}`
    ...
    /**
     * lee datos y llena tabla #datosTabla
     */
    function llenaTabla() {
        ...
        const grid = new gridjs.Grid({
            ...
            search: {
                selector: (cell, rowIndex, cellIndex) => cellIndex === 0 ? cell.descripcion : cell
            },
            columns: ['id','ejemplo','descripcion'],
            server: {
              url: localUrl,
              then: data => data.map(parte => 
                [parte.id, 'ejemplo',parte.descripcion]
              )
            },
            pagination: {
                limit: 9,
    ...
    /**
     * recupera todos los datos, llamada desde el menu
     * @param {*} evt 
     */
